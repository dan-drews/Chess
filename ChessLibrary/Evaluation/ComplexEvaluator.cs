        ...
        for (Files f = Files.A; f <= Files.H; f++)
        {
            for (int rank = 1; rank <= 8; rank++)
            {
                ...
                if (square.Piece?.Type != null && square.Piece.Color == color)
                {
                    ...
                    {
                        case PieceTypes.Knight:
                            score += _knightSquareTable[GetTableLocationForPosition(square.Square.SquareNumber, square.Piece.Color)];
                            break;
                        case PieceTypes.King:
                            score += _kingSquareTable[GetTableLocationForPosition(square.Square.SquareNumber, square.Piece.Color)];
                            break;
                        case PieceTypes.Bishop:
                            score += _bishopSquareTable[GetTableLocationForPosition(square.Square.SquareNumber, square.Piece.Color)];
                            break;
                        case PieceTypes.Queen:
                            if (totalMoveCount < 12)
                            {
                                score += _queenEarlyGameSquareTable[GetTableLocationForPosition(square.Square.SquareNumber, square.Piece.Color)];
                            }
                            else
                            {
                                score += _queenSquareTable[GetTableLocationForPosition(square.Square.SquareNumber, square.Piece.Color)];
                            }
                            break;
                        case PieceTypes.Rook:
                            score += _rookSquareTable[GetTableLocationForPosition(square.Square.SquareNumber, square.Piece.Color)];
                            break;
                        case PieceTypes.Pawn:
                            score += _pawnSquareTable[GetTableLocationForPosition(square.Square.SquareNumber, square.Piece.Color)];
                            break;

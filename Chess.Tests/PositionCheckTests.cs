    ...
    public class PositionCheckTests
    {
        ...
        public static IEnumerable<object[]> MovesList
        {
            get
            {
                ...
                var positions = System.Text.Json.JsonSerializer.Deserialize<List<SampleTest>>(
                    positionRawText
                );
        ...
        [DataTestMethod]
        public void ComputeMovesPosition2(
            int depth,
            int nodes,
            int captures,
            int enPassants,
        ...
        private (int moves, int caputres, int enPassants, int castles) RecurseMoves(
            int depth,
            bool isDebugging
        )
        {
            if (depth == 1)
            {
                ...
                var castles = moves
                    .Where(x => x.Flags == Flag.LongCastle || x.Flags == Flag.ShortCastle)
                    .Count();
                return (
                    moves.Length,
                    moves.Where(x => x.CapturedPiece != null).Count(),
                    ep,
                    castles
                );
            }
